...
def start(message):
    ...
    name = message.text
    bot.send_message(message.from_user.id,
                     f'Glad to meet you, {name}🤗. Shall we begin? '
...
def get_plate(message):
    if message.text == 'yes':
        ...
        bot.send_message(message.from_user.id, text=f'Choose, what {name} like!', reply_markup=keyboard)
    elif message.text == 'no':
...
def get_recipe(message):
    if message.text == 'yes':
        ...
        bot.send_message(message.from_user.id, text=f'Take care, {name}! Come back anytime 🫰🏻')
        bot.send_message(message.from_user.id,
                         text='Copyright © 2011, Harvard University. '
                              'For more information about The Healthy Eating Plate, '
                              'please see The Nutrition Source, Department of Nutrition, '
...
def callback_worker(call):
    if call.data == 'veggies':
        ...
        bot.send_message(call.message.chat.id,
                         f'Do you like it {name}? You can try again until you find THE ONE you need 🫶')
